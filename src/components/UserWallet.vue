    ...
    <template #title>
      <div class="flex w-full items-center justify-between bg-block/50">
        <div
          ...
          <a-dropdown
            @select="handleSelect"
            style="width: 13rem; background-color: rgb(0 0 0 / 50%)">
            <a-button
              style="width: 13rem; color: wheat; background: rgb(0 0 0 / 50%)">
              {{ currentNetWork?.name }}
            </a-button>
            <template #content>
              <a-doption
                v-for="network in netWorkChain"
                :value="network.id"
                :key="network.id"
                :style="{
                  'background-color':
...
const currentNetWork = computed(() =>
  netWorkChain.find((x) => x.id == chainId.value),
)
...
const addressText = computed(() =>
  formatUserAddress((address.value as string) || ''),
)
