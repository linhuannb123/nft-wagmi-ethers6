    ...
    <a-modal v-model:visible="visible" width="400px" @cancel='handleCancel' hideCancel :closable="false" :footer="false"
        ...
        <template #title>
            <div class="flex w-full items-center justify-between bg-block/50">
                <h1 class="flex-1 h-[1.875rem] text-white ml-2 font-bold text-sm leading-[1.875rem] ">
                    Connect your wallet
                </h1>
        ...
        <div class="h-[calc(90vh-8rem)] w-full pb-4">
            <ul class="w-full h-fit flex flex-col mb-4 gap-4 items-center text-xl  font-medium text-gray-600 p-2 px-2">
                <li class="h-[4rem] flex rounded-lg bg-violet-500 text-white hover:bg-[rgba(0,0,0,0.3)]  w-full justify-between items-between p-4 cursor-pointer"
                    v-for="connector in connectors" @click="handleConnect(connector, chainId)" :key="connector.id">

                    <div class="w-fit h-full flex items-center">
                        <img :src="connector.icon || imgMap[connector.type]" :alt="connect.name"
...
const imgModules = import.meta.glob('@/assets/images/wallet/*.(webp|png)', { eager: true, import: 'default' })
...
const imgMap = Object.entries(imgModules).reduce((map: any, [path, module]) => {
